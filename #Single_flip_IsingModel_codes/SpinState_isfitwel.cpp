...
int main( int argc, char** argv ){
    //input factors
    long long int eff_mc_step = -1; long long int criticla_eff_mc_step = -1;
    ...
    for (int i = 1; i < argc; i++){
        ...
        if (arg == "-s"){
            ...
            criticla_eff_mc_step = stoll(argv[i+1]);i += 1;} 
        else if (arg == "-ts"){
    ...
    } if((sqrtN < 0)||(eff_mc_step < 0)||(T_start < 0)||(T_last < 0)||(T_interval < 0 )||(criticla_eff_mc_step < 0)||(T_critical_intv < 0)){cout<<"ERROR:38026asdf89h98hf"<<endl;return 0;}
    //initial position and conditions
    int N = sqrtN*sqrtN; int junk_count = 5000*N; 
    long long int apply_count = N*eff_mc_step; long long int critical_apply_count = N*criticla_eff_mc_step;
    ...
    + "_cteffmc" + to_string(criticla_eff_mc_step) +"_intv" +to_string(T_interval).substr(0, 5) \
    + "_ctit" + to_string(T_critical_intv).substr(0, 6) + addition_name + ".txt"; 
    ...
    for ( ; T >= 2.0 ; T-=T_critical_intv ){
        //apply to mean m
        string line = to_string(T);
        for(int ___ = 0 ; ___ < 10 ; ___++){
            long double m_sum = .0;
            for (long long int _ = 0 ; _ < critical_apply_count / 10 ; _++){
                //random position
                float prob = generator2();
                int position = prob ;
            ...
            line += "," + to_string(m_sum / (critical_apply_count/10));
        }
        //write at writefile
        writefile << line << endl;
    }
    for ( ; T >= T_last ; T-=T_interval ){
        //apply to mean m
        string line = to_string(T);
        for(int ___ = 0 ; ___ < 10 ; ___++){
            long double m_sum = .0;
