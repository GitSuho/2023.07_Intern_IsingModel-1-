...
int main( int argc, char** argv ){
    ...
    //write file generate
    string filename = "Ising_"+to_string(sqrtN)+"x"+to_string(sqrtN)+"_MCeff"+to_string(eff_mc_step)+"_intv"+to_string(T_interval).substr(0, 5)+addition_name+".txt"; 
    ...
    for (float T = T_start ; T >= T_last ; T -= T_interval ){
        ...
        for(int ___ = 0 ; ___ < 10 ; ___++){
            ...
            for (long long int _ = 0 ; _ < apply_count / 10 ; _++){
                ...
                // S_sum += 2*SpinMatrix[Cluster[0]]*Cluster.size();//Update the sum of spins
                m_sum += S_sum / N;//Add the m value
            }
