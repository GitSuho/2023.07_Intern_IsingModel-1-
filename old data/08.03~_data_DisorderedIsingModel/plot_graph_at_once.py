...
for file_name in file_list:
    if ( (not file_name.startswith("DisorderedIsing_")) or (not file_name.endswith(".txt"))):
        continue
...
#plot graph
    h = plt.errorbar(x_values, y_means, yerr=y_stddevs, ecolor='red' , elinewidth=1, label = file_name.split("_p")[1][:-4])
    h.get_children()[1].set_linestyle('--')

    plt.xlabel('T')
    plt.ylabel('<m>')
    plt.suptitle('DisorderedIsingModel_graph', fontsize = 15)
    plt.title(file_name.split("_p")[0][10:] , fontsize = 10)#[6:-4]
    plt.ylim(0, 1.1)
